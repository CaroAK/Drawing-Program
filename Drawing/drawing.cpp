...
void installUnderRoot(ShapeNode* newShape)
{
  ...
  TransformNode* newXfn;
  if (rT != nullptr) {
      newXfn = new TransformNode(sceneRoot, newShape, new Matrix(*rT->getInverse()));
  }
  else {
      newXfn = new TransformNode(sceneRoot, newShape, new Matrix());
  }
...
// he/she has clicked at coordinates (x,y) relative to the center of the 
// window. 
...
int main(int argc, char *argv[])
{
  ...
  //_control87(MCW_EM, MCW_EM); //UNCOMMENT
